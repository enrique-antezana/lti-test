...
# os.environ['CURL_CA_BUNDLE'] = '/home/enrique.antezana/Documents/lti/pylti1.3-flask-example/game/canvas.docker.crt'
# os.environ['REQUESTS_CA_BUNDLE'] = '/home/enrique.antezana/Documents/lti/pylti1.3-flask-example/game/canvas.docker.crt'
...
app = Flask('pylti1p3-game-example', template_folder=BASE_DIR / 'templates', static_folder=BASE_DIR / 'static')
...
def get_lti_config_path():
    # path = os.path.join(app.root_path, '..', 'configs', 'game.json')
    BASE_DIR = Path(__file__).resolve().parent 
    path = BASE_DIR / "config" / "game.json"
    print(f"###################### path: {path}")
...
def login():
    ...
    print(f"###################### /api/login/ target_link_uri: {target_link_uri}")
...
def get_jwks():
    f = open(get_lti_config_path(), 'r')
    key_content = f.read()
    print(f"###################### jwks: {key_content}")
    tool_conf = ToolConfJsonFile(get_lti_config_path())
    return jsonify({'keys': tool_conf.get_jwks()})


@app.route('/api/configure/<launch_id>/<difficulty>/', methods=['GET', 'POST'])
...
def score(launch_id, earned_score, time_spent):
    ...
    print(f"###################### resource_link_id: {resource_link_id}")
    ...
    print(f"###################### earned_score: {earned_score}")
    print(f"###################### sub: {sub}")
    print(f"###################### can_create_lineitem: {grades.can_create_lineitem()}")
    items_lst = grades.get_lineitems()
    print(f"###################### items_lst: {items_lst}")
    sc = Grade()
    ...
    print(f"###################### result: {result.get('body')}")
